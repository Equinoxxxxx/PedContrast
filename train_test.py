...
from tools.loss.contrastive_loss import calc_batch_simi, calc_batch_simi_simple, calc_contrast_loss
from tools.loss.classification_loss import FocalLoss3, margin_entropy_loss, kl_divergence
...
def contrast_epoch(model, 
    ...
    for iter, data in enumerate(tbar):
        ...
        if 'sklt' in modalities:
            inputs['sklt'] = data['obs_skeletons'].to(device)
        ...
        with torch.enable_grad():
            feat_dict = model(inputs)
            simi_mats = calc_batch_simi_simple(feat_dict=feat_dict,
            ...
            # grad
            for m in feat_dict:
                pool_fs[m].append(feat_dict[m].mean().detach().item())
                # if torch.cuda.device_count() <= 1:
                #     pool_f_grads_min[m].append(torch.autograd.grad(loss, feat_dict[m], retain_graph=True)[0].min().detach().item())
                #     pool_f_grads_max[m].append(torch.autograd.grad(loss, feat_dict[m], retain_graph=True)[0].max().detach().item())
        ...
        if isinstance(scheduler, torch.optim.lr_scheduler.OneCycleLR) or batch_schedule:
    ...
    # log res
    log('\n')
    log(f'\t contrast loss: {total_contrast_loss}')
...
def train_test_epoch(
    ...
    for iter, data in enumerate(tbar):
        ...
        with grad_req:
            ...
            if train_or_test == 'train':
                ...
                contrast_loss = 0
                if contrast_eff > 0:
                    simi_mats = simi_mats = calc_batch_simi_simple(feat_dict=feat_dict,
                                               log_logit_scale=model.module.logit_scale,
                                               simi_func=simi_func,
                                               pair_mode=pair_mode
                                               )
                ...
                for m in feat_dict:
                    ...
                    pool_fs[m].append(feat_dict[m].detach().mean().item())
                    # if torch.cuda.device_count() <= 1:
                    #     pool_f_grads_min[m].append(torch.autograd.grad(loss, feat_dict[m], retain_graph=True)[0].min().detach().item())
                    #     pool_f_grads_max[m].append(torch.autograd.grad(loss, feat_dict[m], retain_graph=True)[0].max().detach().item())
                optimizer.zero_grad()
    ...
    if 'cross' in acc_e:
        auc_cross = calc_auc(logits_e['cross'], targets_e['cross'])
        conf_mat = calc_confusion_matrix(logits_e['cross'], targets_e['cross'])
        conf_mat_norm = calc_confusion_matrix(logits_e['cross'], targets_e['cross'], norm='true')
    # return res
